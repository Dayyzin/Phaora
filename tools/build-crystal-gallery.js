...
const spreads = pieces.map((p, i) => {
  ...
  const plateC = plates.length > 5 ? plates[Math.min(6, plates.length - 1)] : null;
...
<article class="cr-spread ${side}" id="${p.slug}">
  ...
  <div class="cr-text">
    ...
    <div class="cr-foot">
      ...
      <span class="cr-more">${plates.length} photographs on file</span>
    </div>
  </div>
</article>`;
}).join('\n');
...
.cr-plate-c{margin-top:18px;width:62%}
.cr-plate-c img{border-radius:10px}
...
.cr-more{color:rgba(234,239,245,.28)}

#!/usr/bin/env node
// Generate all 34 piece pages from assets/sculptures/ catalog
...
const PIECES_OUT    = path.join(ROOT, 'pieces');
const CATALOG_PATH  = path.join(SCULPTURES, 'catalog.json');

// Desired label order for photo grid (hero = full-spread opener)
const LABEL_ORDER = [
  'hero','front','side-right','side-left','back',
  'detail-face','detail-feathers','base-stone','base-detail',
  'scale-context','lifestyle','editorial'
];

// Grid span assignments for photos 0–11
const GRID_SPECS = [
  [8, 5],   // 0  hero — wide
  [4, 5],   // 1  second — tall right
  [5, 4],   // 2
  [4, 4],   // 3
  [3, 4],   // 4
  [12, 3],  // 5  full-width band
  [6, 4],   // 6
  [6, 4],   // 7
  [4, 3],   // 8
  [4, 3],   // 9
...
pieces.forEach((piece, idx) => {
  const { slug, name, price_usd } = piece;
  const folder     = path.join(SCULPTURES, slug);
  const anglemapF  = path.join(folder, '.angle-map.json');
  const hasMap     = fs.existsSync(anglemapF);

  // ── photo list ──────────────────────────────────────────────────────────────
  let photos = fs.readdirSync(folder)
    .filter(f => /\.(jpg|jpeg|png|webp)$/i.test(f) && !f.startsWith('.'))
    .sort();

  if (hasMap) {
    const am = JSON.parse(fs.readFileSync(anglemapF, 'utf8'));
    // Normalise all three schema variants into { originalFilename, label } pairs
    const entries = (am.sequence || am.classifications || am.mappings || []).map(e => ({
      originalFilename: e.original_filename || e.original,
      label: e.label
    }));
    // label → filename
    const byLabel = {};
    for (const entry of entries) byLabel[entry.label] = entry.originalFilename;
    const ordered = [];
    for (const lbl of LABEL_ORDER) {
      if (byLabel[lbl] && photos.includes(byLabel[lbl])) ordered.push(byLabel[lbl]);
    }
    // append any not captured by labels
    for (const f of photos) if (!ordered.includes(f)) ordered.push(f);
    photos = ordered;
  }

  const heroPhoto   = photos[0];
  const heroUrl     = `../assets/sculptures/${slug}/${heroPhoto}`;
  const photoCount  = Math.min(photos.length, 12);
  const displayName = name; // e.g. "The Amethyst Crown"
  const partName    = titleCase(slug); // e.g. "Amethyst Crown"
  const priceDisplay = price_usd > 0
    ? `USD ${price_usd.toLocaleString('en-US')}`
    : 'By appointment';
  const subjectEnc = encodeURIComponent(`Enquiry: ${displayName}`);

  // ── prev / next ─────────────────────────────────────────────────────────────
  const prevIdx  = (idx - 1 + slugs.length) % slugs.length;
  const nextIdx  = (idx + 1) % slugs.length;
  const prevSlug = slugs[prevIdx];
  const nextName = pieces[nextIdx].name;
  const prevName = pieces[prevIdx].name;

  // ── photo grid tiles ────────────────────────────────────────────────────────
  const gridTiles = photos.slice(0, 12).map((photo, i) => {
    const [col, row] = GRID_SPECS[i] || [4, 3];
    const cap = `${displayName} · Plate ${i + 1}`;
    const src = `../assets/sculptures/${slug}/${photo}`;
...
<nav class="nav"><div class="nav-left"><a href="../hardscape-gallery.html">Hardscape</a><a href="https://phaora.com/sculptures.html">Sculpture</a><a href="../gallery.html">Gallery</a><a href="../agents.html">Agents</a></div><a href="../index.html" class="nav-logo">PHAÖRA</a><div class="nav-right"><a href="../index.html#studio">Studio</a><a href="../contact.html">Contact</a><a href="../index.html#invitation" class="nav-enquire">Enquire <span class="arrow">&rarr;</span></a></div></nav>

<div class="crumb"><a href="https://phaora.com/sculptures.html">&larr; Back to the collection</a><span class="crumb-num">${String(idx + 1).padStart(2, '0')} of Thirty-four</span></div>

<!-- HERO -->
<section class="piece-hero gf">
  <div class="piece-hero-bg" style="background-image:url('${heroUrl}')"></div>
  <div class="piece-super">Piece &middot; ${String(idx + 1).padStart(2, '0')}</div>
  <h1>The <em>${partName}</em></h1>
  <p class="piece-sub">Crystal sculpture &middot; Minas Gerais, Brazil</p>
...
<section class="piece-body reveal">
  <div class="piece-body-inner">
    <div class="meta-card">
      <div class="mc-price"><span class="mc-price-label">Price</span><div class="mc-price-val">${priceDisplay}</div><span class="mc-price-ed">One of one</span></div>
      <div class="mc-row"><span class="mc-label">Medium</span><span class="mc-val">Brazilian crystal</span></div>
      <div class="mc-row"><span class="mc-label">Origin</span><span class="mc-val">Minas Gerais, Brazil</span></div>
      <div class="mc-row"><span class="mc-label">Edition</span><span class="mc-val">One of one</span></div>
...
<section class="photos reveal">
  <div class="photos-label">${photoCount} photograph${photoCount !== 1 ? 's' : ''} &middot; ${displayName}</div>
  <div class="p-grid">
${gridTiles}
  </div>
</section>
...
<!-- PREV / NEXT -->
<nav class="pn"><a href="${prevSlug}.html"><span class="pn-label">&larr; Previous</span><span class="pn-name">${prevName}</span></a><a href="${slugs[nextIdx]}.html" class="pn-next"><span class="pn-label">Next &rarr;</span><span class="pn-name">${nextName}</span></a></nav>

<!-- CORRESPONDENCE -->
<section class="corr gf reveal">
  <div class="corr-inner">
    <span class="super-label">&mdash; Correspondence &mdash;</span>
    <h3>Enquire about <em>${displayName}</em></h3>
...
(function(){
  ...
  var tiles=Array.from(document.querySelectorAll('.g-tile'));
  if(!tiles.length)return;
  var lb=document.getElementById('lb'),lbImg=document.getElementById('lb-img'),lbTitle=document.getElementById('lb-title'),lbSub=document.getElementById('lb-sub'),lbCounter=document.getElementById('lb-counter');
  var idx=0,total=tiles.length;
  function show(i){idx=((i%total)+total)%total;var t=tiles[idx],cap=t.getAttribute('data-cap')||'',parts=cap.split(' \u00B7 ');lbImg.src=t.querySelector('img').src;lbTitle.textContent=parts[0]||'';lbSub.textContent=parts[1]||'';lbCounter.textContent=(idx+1)+' / '+total}
  ...
  const outPath = path.join(PIECES_OUT, `${slug}.html`);
  fs.writeFileSync(outPath, html, 'utf8');
  const mapNote = hasMap ? 'angle-map' : 'filename order';
